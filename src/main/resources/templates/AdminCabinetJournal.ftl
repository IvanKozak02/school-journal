  ...
  <div id="content" >
    ...
    <div class="sem-and-subject" style="padding-top: 20px">
      <div class="input-field class-name" style="padding-top: 15px; padding-left: 40px; width: 15%">
        <label>Клас
          <select onchange="getSubjects()" class="class-select" style="width: 96%" name="sex" required>
      ...
      <div class="input-field subject-name" style="padding-top: 15px; padding-left: 40px; width: 15%">
        <label>Предмет
          <select onchange="getGroup()" class="class-teacher subject-select" style="width: 96%" name="sex" required>
            <option selected disabled>Оберіть предмет</option>
          </select>
        </label>
      </div>
      <div class="input-field class-group" style="padding-top: 15px; padding-left: 40px; width: 15%; display: none;">
        <label>Група
          <select onchange="getSubjectJournal()" class="class-teacher group-select" style="width: 96%" name="sex" required>
            <option selected disabled>Оберіть групу</option>
          </select>
        </label>
      </div>
  </div>
    <div class="img-container" style="align-items: center; flex-direction: column; display:flex;">
      ...
      <h1 class="class-user-name" style="padding-top: 30px;">Для відображення Обліку навчальних досягнень, оберіть Клас, Предмет та Групу, за потреби</h1>
    </div>
</div>
</div>
